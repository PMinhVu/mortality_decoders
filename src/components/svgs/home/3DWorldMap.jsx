...
import WorldMapData from '../../../assets/data/world.js'; // Ensure this is correctly imported as a JS file
import MortalityForAll from '../../../assets/data/Combined_Dataset.csv'; // Ensure CSV is correctly imported
...
const SphereWorldMap = ({ country, year, indicator }) => {
    ...
    useEffect(() => {
        ...
        // Process data
        const filteredMortalityData = MortalityForAll.filter(
            (d) => +d.REF_DATE === year && d.Indicator === indicator
        );

        const mortalityByArea = new Map(
            filteredMortalityData.map((d) => [
                d.Area,
        ...
        svg.append('g')
            ...
            .attr('fill', (d) => {
                const data = mortalityByArea.get(d.properties.name);
        ...
        const zoomToCountry = (countryName) => {
            const selectedCountry = WorldMapData.features.find((d) => d.properties.name === countryName);
            if (selectedCountry) {
